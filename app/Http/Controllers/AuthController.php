...
class AuthController extends Controller
{
    ...
    public function proses_login(Request $request) {
        ...
        // cek jika ada data username dan password valid (sesuai) dengan data
        if (Auth::attempt($credential)) {
            
            //kalau berhasil simpan data user di variabel $user
            $user = Auth::user();
            
            //cek lagi jika level user admin maka akan diarahkan ke halaman admin
            if ($user->level_id == '1') {
                return redirect()->intended('admin');
            }
            //tapi jika level user hany user biasa maka arahkan ke halaman user
            if ($user->level_id == '2') {
                return redirect()->intended('manager');
            }
    ...
    public function proses_register(Request $request) {
        ...
        //ketika berhasil arahkan ke halaman login
